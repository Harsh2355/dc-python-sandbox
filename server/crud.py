...
def submit_code(
    db: Session,
    python_code: schemas.PythonCode,
    execution_output: schemas.ExecutionOutput,
) -> schemas.ExecutionOutput:
    code_to_submit = models.SubmittedCode(
        code=python_code.code, output=execution_output.message
    )
